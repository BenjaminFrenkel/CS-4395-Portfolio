...
def find_topic(text: str):
    ...
    # Remove stop words
    global stop_words
    stop_words_extra = stop_words.copy()
    stop_words_extra.add('dislike'); stop_words_extra.add('like'); stop_words_extra.add('enjoy'); stop_words_extra.add('love'); stop_words_extra.add('loathe')
    tokenized_text = [token for token in tokenized_text if token not in stop_words_extra]

    # POS tag the tokenized text
    # print(tokenized_text) # debug
    tagged_text = pos_tag(tokenized_text, tagset='universal')
...
def chatbot(model: LogisticRegression, responses: dict, tag_encoding: dict, user_name=""):
    ...
    while True:
        ...
        if user_input.lower() in exit_inputs:
            # Leave program
            print("Thank you for your time" + ('.' if user_name == '' else ', ' + user_name + '! ') +
        ...
        if conversation_active:
        ...
        elif 'conversation_' in tag:
            # Set conversation active to true (expecting response, context matters)
            conversation_active = True

            # Get response
            response_to_print: str = response[response_index]

            if "<topic>" in response_to_print:
                # Determine topic
                topic = find_topic(user_input_copy2)
                if topic != '':
                    response_to_print = response_to_print.replace("<topic>", topic)
                else:
            ...
            elif "<remark>" in response_to_print:
                ...
                if random_remark == 0 and len(user_model.life_info) > 0:
                    ...
                    response_to_print = response_to_print.replace("<remark>", "I recall you said, \"" + chosen_remark +
                    ...
                    response_to_print = response_to_print.replace("<remark>", "I don't think I've heard much about you"
...
def main():
    ...
    """
    # Grab globals
    global data_folder, kb_file, pickled_model_r, pickled_model_w
